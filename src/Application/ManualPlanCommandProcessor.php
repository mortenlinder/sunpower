...
final class ManualPlanCommandProcessor
{
    ...
    public function tick():void
    {
        ...
        try{$payload=json_decode($command['payload_json'],true,32,JSON_THROW_ON_ERROR);$schedule=(new GrowattWindowCompiler($this->pdo))->compile((int)$payload['plan_id']);$result=$this->control->applySchedule($schedule);$result['schedule']=$schedule;
            $this->state('manual_schedule',json_encode($schedule,JSON_THROW_ON_ERROR),'Planen er manuelt anvendt og udløber automatisk');$this->audit('apply_approved_plan',(string)$payload['plan_id'],$result,'Manuelt anvendt plan; ingen automatisk replanning');$done=$this->pdo->prepare("UPDATE commands SET status='verified',completed_at=UTC_TIMESTAMP(6),error_message=NULL WHERE id=?");$done->execute([$command['id']]);
