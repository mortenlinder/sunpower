...
final class AutomaticPlanService
{
    ...
    public function run(?int $planId):array
    {
        ...
        if($settings['last_auto_price_until']===null&&$settings['last_auto_plan_date']===$today){$store->put('last_auto_price_until',(string)$priceUntil,'Eksisterende dagsplan knyttet til aktuel prishorisont');return['status'=>'already_planned','date'=>$today,'price_until'=>$priceUntil];}
        if($settings['last_auto_price_until']!==null&&strtotime((string)$priceUntil)<=strtotime($settings['last_auto_price_until']))return['status'=>'waiting_for_new_prices','price_until'=>$priceUntil];
