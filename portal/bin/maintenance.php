...
if (!in_array($config['mail_transport']??'',['sendmail','smtp'],true) || !filter_var($config['mail_from'],FILTER_VALIDATE_EMAIL)) throw new RuntimeException('Configure mail delivery');
$mailer=new SolportalCloud\Mailer($config);
...
foreach ($q as $mail) {
    try { $ok=$mailer->send($mail['recipient'],$mail['subject'],$mail['body']); }
    catch (Throwable $error) { $ok=false; error_log('Solportal mail configuration or transport failure'); }
