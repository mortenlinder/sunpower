...
$fallbackLabel = ($automationSettings['fallback_mode'] ?? 'battery_first') === 'battery_first' ? 'Battery First' : 'Load First';
$automationEnabled = (bool)($automationSettings['enabled'] ?? false);
?>
<!doctype html>
...
<head>
  ...
  <link rel="manifest" href="/manifest.webmanifest"><link rel="stylesheet" href="/assets/css/app.css?v=9"><link rel="stylesheet" href="/assets/css/insights.css"><link rel="stylesheet" href="/assets/css/learning.css"><link rel="stylesheet" href="/assets/css/plan.css"><link rel="stylesheet" href="/assets/css/control.css?v=9">
...
<main>
  ...
  <section class="intelligence" id="intelligent"><span id="weather"></span><span id="prices"></span>
    ...
    <article class="plan-card"><div class="plan-head"><span class="brain">✦</span><div><p class="eyebrow">INTELLIGENT PLAN</p><h2><?= $automationEnabled?'Automatisk drift':'Manuel drift' ?></h2></div><span class="readonly-badge"><?= $automationEnabled?'AKTIV':($writesEnabled?'KLAR':'INGEN WRITES') ?></span></div><p class="plan-lead" id="plan-action">Afventer pris- og vejrdata.</p><div class="timeline" id="plan-timeline"></div><div class="consumption-watch"><span>⌁</span><div><b>Forbrugsvagten lærer</b><small id="consumption-status">Ingen elbilhændelser genkendt endnu</small></div></div><div class="plan-note"><span>✓</span><p><b><?= $automationEnabled?'Rullende planlægning':'Styring efter godkendelse' ?></b><small><?= $automationEnabled?'Planen forlænges automatisk, når nye priser er tilgængelige.':'Aktivér intelligent styring i panelet, eller anvend planen manuelt.' ?></small></p></div></article>
  </section>
  ...
  <section class="full-plan" id="plan">
    ...
    <div class="approval-panel"><div><span class="approval-icon">✓</span><div><b id="approval-title"><?= $automationEnabled?'Automatisk plan':'Gennemse og gem planen' ?></b><small id="approval-copy">Efter planens udløb vælges <?= htmlspecialchars($fallbackLabel,ENT_QUOTES) ?> automatisk.</small></div></div><div class="approval-buttons"><a class="settings-link" href="/settings/automation">Styringsindstillinger</a><button id="approve-plan" type="button" disabled>Gem og godkend planen</button><button id="apply-plan" type="button" disabled>Anvend på inverter</button></div></div>
  </section>
